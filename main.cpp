...
//#pragma omp parallel for num_threads(2) default(none), shared(n, d, k, randclass, is_prime)
	for (int i = 0; i < k; i++) {
		if (!is_prime) {
			continue;
		}
#pragma task default(none), shared(n, d, randclass, is_prime)
...
#ifndef TEST_SQRT
#pragma omp parallel for schedule(dynamic) default(none), shared(n, items, k, r)
	for (int i = 0; i < n; i++) {
		mpz_class a(0);
		string & item = items[i];
		//printf("num: %lu\n", item.length());
